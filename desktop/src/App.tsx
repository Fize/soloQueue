  ...
  return (
    <TooltipProvider>
      ...
      <div className="flex h-screen w-screen bg-background overflow-hidden select-none relative">
        ...
        <div className="absolute left-[78px] top-0 z-[100] h-12 flex items-center justify-center px-0.5">
          <button
            onClick={toggleCollapse}
            onMouseEnter={handleHoverEnter}
            onMouseLeave={handleHoverLeave}
            className="electron-no-drag flex items-center justify-center rounded-md p-1.5 transition-colors duration-150 hover:bg-foreground/10 text-muted-foreground hover:text-foreground shrink-0 cursor-pointer"
            title={effectivelyCollapsed ? '展开侧边栏' : '收起侧边栏'}
          >
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div
            className={cn(
              'shrink-0 h-full border-r border-border/40 bg-card/40 backdrop-blur-md transition-all duration-300 ease-out relative',
              effectivelyCollapsed ? 'w-[52px]' : 'w-[220px]'
            )}
            onMouseEnter={handleHoverEnter}
            onMouseLeave={handleHoverLeave}
          >
          ...
          <main className="flex flex-1 flex-col min-w-0 overflow-hidden h-full bg-background relative">
            ...
            <div className="h-8 electron-drag-region bg-background/20 shrink-0" />
