...
import { Plus, AlertCircle, Loader2, X, Workflow, Clock, RefreshCw } from 'lucide-react'
...
function CreateSheet({ open, onClose, onCreated }: CreateSheetProps) {
  ...
  const { availableAgents, availableAgentsLoading, availableAgentsError, fetchAvailableAgents } =
    useWorkflowStore()
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const unknownAgents = parsed ? unknownAgentTemplates(parsed.agents, availableAgents) : []
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-[480px] flex flex-col bg-card border-l border-border shadow-2xl transition-transform duration-300 ease-out translate-x-0">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <form id="create-workflow-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                rows={16}
                placeholder={defaultYAMLTemplate(
                  name || 'my-workflow',
                  effectiveSelectedAgent || undefined
                )}
                value={yaml}
        ...
        <div className="shrink-0 px-6 py-4 border-t border-border/60 bg-card/60">
          <button
            ...
            disabled={
              creating || !name.trim() || availableAgentsLoading || availableAgents.length === 0
            }
...
export function WorkflowListPage() {
  ...
  }, [fetchWorkflowMetas, fetchBuiltinWorkflows])

  useEffect(() => {
    workflowMetas.forEach((workflow) => {
      void fetchRuns(workflow.name)
    })
  ...
  useEffect(() => {
    ...
    if (workflowMetasLoading) return
    void fetchWorkflowMetas()
    void fetchBuiltinWorkflows()
    workflowMetas.forEach((workflow) => {
      void fetchRuns(workflow.name)
  ...
  const activeRuns = allRuns.filter((r) =>
    ['running', 'preparing_worktree', 'pause_requested', 'resuming'].includes(r.status)
  )
      ...
      <div className="flex h-full flex-col overflow-hidden bg-background text-foreground">
        ...
        >
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">{t('workflow.description')}</p>
          </div>
          <div className="flex items-center gap-3 electron-no-drag">
            <button
              onClick={fetchWorkflowMetas}
              disabled={workflowMetasLoading || !backendReady}
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider font-mono">
                      {t('workflow.progress', {
                        completed: activeRuns.reduce((s, r) => s + r.completed_count, 0),
                        total: activeRuns.reduce((s, r) => s + r.node_count, 0),
                      })}
                    </h2>
                  </div>
