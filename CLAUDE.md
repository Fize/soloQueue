...
- **Message types** (`api.go`): constants `MsgTypeText=0`, `MsgTypeMarkdown=2`, `MsgTypeArk=3`, `MsgTypeEmbed=4`, `MsgTypeMedia=7`. `MessageReq` uses typed payload fields (`Markdown *MarkdownContent`, `Media *MediaContent`, etc.) with `omitempty` so only the field matching `msg_type` is serialized. `buildMessageReq(msgType, content, msgID, msgSeq)` constructs the correct payload shape.
- **Rich media** (`api.go`): two-step upload flow — `UploadFile(ctx, targetType, targetID, fileType, url)` → returns `FileInfo` with `file_info` string → send via `MsgTypeMedia`. File type constants: `FileTypeImage=1`, `FileTypeVideo=2`, `FileTypeVoice=3`, `FileTypeFile=4`.
- **`SessionBridge`** (`handler.go`): receives parsed QQ messages from Gateway, calls `SessionAskAdapter.AskStream()`, formats reply via `QQMarkdown()`, sends as `MsgTypeMarkdown`. No redundant concurrency guard — Session's `inFlight` already serializes. System messages (busy, error, empty reply) use `MsgTypeText`.
- **`SessionProvider`** interface (`handler.go`): decouples qqbot from the session package (avoids import cycles). Defines `AskStream(ctx, prompt) (*AskStreamResult, error)`. Implemented by `SessionAskAdapter` in `session/qqbot_adapter.go`.
- **Markdown converter** (`markdown.go`): `QQMarkdown(input)` transforms LLM CommonMark output to QQ-compatible markdown — fenced code blocks → bold `**lang**` header + indented body, inline code → bold, tables → bulleted lists. Preserves headings, bold, italic, strikethrough, links, images, lists, block quotes. `SplitMarkdown(text, maxLen)` splits long responses at heading boundaries, then paragraph breaks, then line breaks for QQ's ~2000-byte message limit.
