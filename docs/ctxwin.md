...
4. **Ordering Rule**: Calibration must run *before* pushing subsequent assistant replies or tool results to ensure that FIFO eviction calculations subtract accurate numbers.
...
- For turns *before* the boundary, if a message is marked `IsEphemeral` (e.g., raw files or terminal outputs), the system strips its content:
