...
-- workflow_runs stores the current durable run snapshot. Append-only history
-- lives in workflow_run_events and per-attempt details in workflow_node_runs.
CREATE TABLE IF NOT EXISTS workflow_runs (
	...
	restarted_from_run_id TEXT NOT NULL DEFAULT '',
	...
	error_message TEXT NOT NULL DEFAULT '',
	audit_dir TEXT NOT NULL DEFAULT '',
...
CREATE INDEX IF NOT EXISTS idx_workflow_run_events_run ON workflow_run_events(workflow_run_id, id);
CREATE INDEX IF NOT EXISTS idx_workflow_run_events_type ON workflow_run_events(workflow_run_id, event_type);
...
CREATE TABLE IF NOT EXISTS workflow_run_checkpoints (
	...
	ready_queue_json TEXT NOT NULL DEFAULT '[]',
	join_buckets_json TEXT NOT NULL DEFAULT '{}',
	loop_counters_json TEXT NOT NULL DEFAULT '{}',
	completed_node_runs_json TEXT NOT NULL DEFAULT '[]',
	worktree_head TEXT NOT NULL DEFAULT '',
	worktree_status_hash TEXT NOT NULL DEFAULT '',
	worktree_patch_hash TEXT NOT NULL DEFAULT '',
	created_at TEXT NOT NULL DEFAULT (datetime('now')),
	UNIQUE(workflow_run_id, sequence)
);
CREATE INDEX IF NOT EXISTS idx_workflow_run_checkpoints_run ON workflow_run_checkpoints(workflow_run_id, sequence DESC);
...
CREATE INDEX IF NOT EXISTS idx_workflow_worktrees_run ON workflow_worktrees(workflow_run_id, kind);
...
CREATE TABLE IF NOT EXISTS workflow_confirmations (
	...
	allow_in_session INTEGER NOT NULL DEFAULT 0,
...
func (d *DB) migrate() error {
	...
	// v13: extend workflow_runs in place for databases created by v12. The
	// snapshot above covers fresh databases; existing SQLite tables require
	// explicit ALTER TABLE statements because CREATE TABLE IF NOT EXISTS does
	// not add columns.
	workflowRunColumns := []struct {
		name string
		ddl  string
	}{
		{"workflow_version", `ALTER TABLE workflow_runs ADD COLUMN workflow_version TEXT NOT NULL DEFAULT ''`},
		{"workflow_hash", `ALTER TABLE workflow_runs ADD COLUMN workflow_hash TEXT NOT NULL DEFAULT ''`},
		{"workflow_yaml", `ALTER TABLE workflow_runs ADD COLUMN workflow_yaml TEXT NOT NULL DEFAULT ''`},
		{"task_json", `ALTER TABLE workflow_runs ADD COLUMN task_json TEXT NOT NULL DEFAULT '{}'`},
		{"work_dir", `ALTER TABLE workflow_runs ADD COLUMN work_dir TEXT NOT NULL DEFAULT ''`},
		{"source", `ALTER TABLE workflow_runs ADD COLUMN source TEXT NOT NULL DEFAULT ''`},
		{"repository_path", `ALTER TABLE workflow_runs ADD COLUMN repository_path TEXT NOT NULL DEFAULT ''`},
		{"base_ref", `ALTER TABLE workflow_runs ADD COLUMN base_ref TEXT NOT NULL DEFAULT ''`},
		{"base_commit", `ALTER TABLE workflow_runs ADD COLUMN base_commit TEXT NOT NULL DEFAULT ''`},
		{"branch_name", `ALTER TABLE workflow_runs ADD COLUMN branch_name TEXT NOT NULL DEFAULT ''`},
		{"worktree_path", `ALTER TABLE workflow_runs ADD COLUMN worktree_path TEXT NOT NULL DEFAULT ''`},
		{"worktree_state", `ALTER TABLE workflow_runs ADD COLUMN worktree_state TEXT NOT NULL DEFAULT ''`},
		{"parent_run_id", `ALTER TABLE workflow_runs ADD COLUMN parent_run_id TEXT NOT NULL DEFAULT ''`},
		{"restarted_from_run_id", `ALTER TABLE workflow_runs ADD COLUMN restarted_from_run_id TEXT NOT NULL DEFAULT ''`},
		{"successor_run_id", `ALTER TABLE workflow_runs ADD COLUMN successor_run_id TEXT NOT NULL DEFAULT ''`},
		{"checkpoint_sequence", `ALTER TABLE workflow_runs ADD COLUMN checkpoint_sequence INTEGER NOT NULL DEFAULT 0`},
		{"pause_mode", `ALTER TABLE workflow_runs ADD COLUMN pause_mode TEXT NOT NULL DEFAULT ''`},
		{"resume_available", `ALTER TABLE workflow_runs ADD COLUMN resume_available INTEGER NOT NULL DEFAULT 0`},
		{"quality_status", `ALTER TABLE workflow_runs ADD COLUMN quality_status TEXT NOT NULL DEFAULT ''`},
		{"delivery_status", `ALTER TABLE workflow_runs ADD COLUMN delivery_status TEXT NOT NULL DEFAULT ''`},
		{"delivery_json", `ALTER TABLE workflow_runs ADD COLUMN delivery_json TEXT NOT NULL DEFAULT '{}'`},
		{"error_code", `ALTER TABLE workflow_runs ADD COLUMN error_code TEXT NOT NULL DEFAULT ''`},
		{"error_message", `ALTER TABLE workflow_runs ADD COLUMN error_message TEXT NOT NULL DEFAULT ''`},
		{"audit_dir", `ALTER TABLE workflow_runs ADD COLUMN audit_dir TEXT NOT NULL DEFAULT ''`},
		{"audit_head_hash", `ALTER TABLE workflow_runs ADD COLUMN audit_head_hash TEXT NOT NULL DEFAULT ''`},
		{"created_at", `ALTER TABLE workflow_runs ADD COLUMN created_at TEXT NOT NULL DEFAULT ''`},
		{"updated_at", `ALTER TABLE workflow_runs ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''`},
	}
	for _, column := range workflowRunColumns {
		hasColumn, err := tableHasColumn(tx, "workflow_runs", column.name)
		if err != nil {
			_ = tx.Rollback()
			return fmt.Errorf("inspect workflow_runs %s column: %w", column.name, err)
		}
		if !hasColumn {
			if _, err := tx.Exec(column.ddl); err != nil {
				_ = tx.Rollback()
				return fmt.Errorf("migrate workflow_runs add %s: %w", column.name, err)
			}
		}
	}
	confirmationColumns := []struct {
		name string
		ddl  string
	}{
		{"options_json", `ALTER TABLE workflow_confirmations ADD COLUMN options_json TEXT NOT NULL DEFAULT '[]'`},
		{"allow_in_session", `ALTER TABLE workflow_confirmations ADD COLUMN allow_in_session INTEGER NOT NULL DEFAULT 0`},
	}
	for _, column := range confirmationColumns {
		hasColumn, err := tableHasColumn(tx, "workflow_confirmations", column.name)
		if err != nil {
			_ = tx.Rollback()
			return fmt.Errorf("inspect workflow_confirmations %s column: %w", column.name, err)
		}
		if !hasColumn {
			if _, err := tx.Exec(column.ddl); err != nil {
				_ = tx.Rollback()
				return fmt.Errorf("migrate workflow_confirmations add %s: %w", column.name, err)
			}
		}
	}
	if _, err := tx.Exec(`
		UPDATE workflow_runs
		SET created_at = CASE WHEN created_at = '' THEN started_at ELSE created_at END,
			updated_at = CASE WHEN updated_at = '' THEN COALESCE(NULLIF(finished_at, ''), started_at) ELSE updated_at END
		WHERE created_at = '' OR updated_at = ''
	`); err != nil {
		_ = tx.Rollback()
		return fmt.Errorf("migrate workflow_runs timestamps: %w", err)
	}
	if _, err := tx.Exec(`
		UPDATE workflow_runs
	...
	`); err != nil {
		_ = tx.Rollback()
		return fmt.Errorf("normalize workflow_runs timestamps: %w", err)
	}
