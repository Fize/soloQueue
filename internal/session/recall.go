...
func (s *Session) buildRecalledContext(ctx context.Context, prompt string) string {
	...
	result := formatRecallResults(targets)
	for _, r := range targets {
		s.recalledHashes[r.ContentHash] = struct{}{}
	}
	return result
}

// BuildRecalledContext searches the memory engine for context relevant to
// the given prompt and returns a formatted text block suitable for prepending
// to a prompt. Unlike the session-scoped buildRecalledContext, this version
// does NOT track recalled hashes (no dedup) — each call is a fresh search.
func BuildRecalledContext(ctx context.Context, prompt string, memEngine *memoryengine.Engine, log *logger.Logger) string {
	if memEngine == nil {
		return ""
	}
	...
	results, err := memEngine.Search(ctxTimeout, memoryengine.SearchQuery{
		Text:  trimmed,
		Limit: 5,
	})
	if err != nil {
		if log != nil {
			log.WarnContext(ctx, "preload recall: search failed",
				"err", err.Error(),
				"prompt_len", len(trimmed),
			)
		}
	...
	return formatRecallResults(results.Results)
}

// formatRecallResults formats search results into the <recalled_memories> block
// with age labels and budget management. Used by both buildRecalledContext and
// BuildRecalledContext to avoid code duplication.
func formatRecallResults(targets []memoryengine.SearchResult) string {
