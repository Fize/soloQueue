...
const (
	codexMessageText       codexMessageType = "text"
	codexMessageToolUse    codexMessageType = "tool_use"
	codexMessageToolResult codexMessageType = "tool_result"
	codexMessageStatus     codexMessageType = "status"
)
...
type codexLogger struct {
	inner *logger.Logger
}

func (l codexLogger) Info(msg string, args ...any) {
	if l.inner != nil {
		l.inner.Info(logger.CatActor, msg, args...)
	}
}

func (l codexLogger) Debug(msg string, args ...any) {
	if l.inner != nil {
		l.inner.Debug(logger.CatActor, msg, args...)
	}
}

func (l codexLogger) Warn(msg string, args ...any) {
	if l.inner != nil {
		l.inner.Warn(logger.CatActor, msg, args...)
	}
}

func (l codexLogger) Error(msg string, args ...any) {
	if l.inner != nil {
		l.inner.Error(logger.CatActor, msg, args...)
	}
}
...
// codexBackend implements externalBackend by spawning `codex app-server --listen stdio://`
...
func buildCodexArgs(opts externalExecOptions, logger codexLogger) []string {
	args := []string{"app-server", "--listen", "stdio://"}
	args = append(args, filterCodexArgs(opts.CustomArgs, codexBlockedArgs, logger.inner)...)
	return args
}

func filterCodexArgs(args []string, blocked map[string]blockedArgMode, l *logger.Logger) []string {
	if len(args) == 0 {
		return args
	}
	...
	for _, arg := range args {
		...
		if isBlocked {
			if l != nil {
				l.Warn(logger.CatActor, "custom_args: blocked protocol-critical flag, skipping", "flag", flag)
			}
...
func (b *codexBackend) Execute(ctx context.Context, prompt string, opts externalExecOptions) (*externalSession, error) {
	execPath := b.execPath
	if execPath == "" {
		execPath = "codex"
	}
	...
	cfg := codexConfig{
		...
		Logger:         codexLogger{inner: b.logger},
		Env:            opts.CustomEnv,
	}

	codexArgs := buildCodexArgs(opts, cfg.Logger)
	cmd := exec.CommandContext(runCtx, execPath, codexArgs...)
	hideAgentWindow(cmd)
	cfg.Logger.Info("agent command", "exec", execPath, "args", codexArgs)
	if opts.Cwd != "" {
		cmd.Dir = opts.Cwd
	}
	...
	stderrBuf := newStderrTail(newLogWriter(cfg.Logger.inner, "[codex:stderr] "), codexStderrTailBytes)
	...
	cfg.Logger.Info("codex started app-server", "pid", cmd.Process.Pid, "cwd", opts.Cwd)
	...
	c = &codexClient{
		...
		onMessage: func(msg codexMessage) {
			logCodexAgentMessage(cfg.Logger, msg)
			if msg.Type == codexMessageText {
				outputMu.Lock()
				output.WriteString(msg.Content)
				outputMu.Unlock()
			}
			
			var extType externalMessageType = externalMessageText
			switch msg.Type {
			case codexMessageText:
				extType = externalMessageText
			case codexMessageStatus:
				extType = externalMessageStatus
			}
	...
	go func() {
		...
		if err != nil {
			...
			resCh <- externalResult{Status: finalStatus, Error: finalError}
			return
		}
		...
		threadID, resumed, err := c.startOrResumeThread(runCtx, opts, cfg.Logger)
		if err != nil {
			drainAndWait() // flush os/exec stderr goroutine before sampling Tail
			finalStatus = "failed"
		...
		c.threadID = threadID
		if resumed {
			cfg.Logger.Info("codex thread resumed", "thread_id", threadID)
		} else {
			cfg.Logger.Info("codex thread started", "thread_id", threadID)
		}
		...
		for waitingForTurn {
			...
			case <-semanticTimer.C:
				...
				cfg.Logger.Warn("codex semantic inactivity timeout",
					"pid", cmd.Process.Pid,
					"thread_id", threadID,
					"turn_id", c.turnID,
		...
		cfg.Logger.Info("codex finished", "pid", cmd.Process.Pid, "status", finalStatus, "duration", duration.Round(time.Millisecond).String())
...
func (c *codexClient) startOrResumeThread(ctx context.Context, opts externalExecOptions, logger codexLogger) (string, bool, error) {
...
func logCodexAgentMessage(logger codexLogger, msg codexMessage) {
	if logger.inner == nil {
		return
	}
	attrs := []any{
	...
	if msg.Type == codexMessageToolResult {
		logger.Info("codex tool_result observed", "tool", msg.Tool, "call_id", msg.CallID, "output_len", len(msg.Output))
	}
}

func describeCodexSemanticActivity(msg codexMessage) string {
	switch msg.Type {
	case codexMessageToolUse, codexMessageToolResult:
		if msg.Tool != "" {
...
func (c *codexClient) handleEvent(msg map[string]any) {
	...
	case "task_started":
		c.turnStarted = true
		if c.onMessage != nil {
			c.onMessage(codexMessage{Type: codexMessageStatus, Status: "running", SessionID: c.threadID})
		}
	case "agent_message":
		text, _ := msg["message"].(string)
		if text != "" && c.onMessage != nil {
			c.onMessage(codexMessage{Type: codexMessageText, Content: text})
		}
	case "exec_command_begin":
		callID, _ := msg["call_id"].(string)
		command, _ := msg["command"].(string)
		if c.onMessage != nil {
			c.onMessage(codexMessage{
				Type:   codexMessageToolUse,
				Tool:   "exec_command",
				CallID: callID,
	...
	case method == "item/completed" && itemType == "fileChange":
		if c.onMessage != nil {
			c.onMessage(codexMessage{
				...
				Tool:   "patch_apply",
				CallID: itemID,
				Output: "", // file changes don't have output in v2
			})
		}
