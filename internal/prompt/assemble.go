...
func assembleWithXML(profile, userCtx, recentMemory, permanentMemory, routingTable, teamMgmt, rules, planDir, workDir, exploreDir string, mcpServers []string) string {
	...
	fmt.Fprintf(&b, "\n\n%s", EnvSection(workDir, exploreDir, true))
	...
	fmt.Fprintf(&b, "\n\n<delegation_requirement>\nYou are a task router, NOT an executor. You MUST use delegate_* tools for ALL tasks. Built-in tools (Read, Bash, Write, Edit, Grep, Glob, WebFetch, WebSearch) are STRICTLY FORBIDDEN — calling them violates protocol. Every user request must be delegated to a team leader.\n\nCRITICAL: When delegating, you MUST include the `work_dir` parameter set to the appropriate workspace path from the team's workspace list. Choose the workspace whose path best matches the task's project location. This ensures the delegated agent works in the correct project directory and loads project-specific configuration (AGENTS.md, CLAUDE.md, .claude/).\n\nExample: delegate_dev(task=\"Fix the login page CSS\", work_dir=\"/path/to/project\")\n</delegation_requirement>")
