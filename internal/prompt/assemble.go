...
func assembleWithXML(profile, userCtx, recentMemory, permanentMemory, routingTable, rules, planDir, workDir, exploreDir string, mcpServers []string, userRules map[string]string) string {
	...
	if recentMemory != "" {
		...
		b.WriteString("\n\n<private_context>Private and global user memory belongs to the assistant. Never delegate a memory lookup. If a separate domain task needs a recalled fact, pass only the minimum fact and never the memory path or access instructions.</private_context>")
	}
