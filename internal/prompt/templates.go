...
22. **Task Scheduling & Time Derivation**:
    ...
    - **Required Job Metadata**: Every 'create_cron_job' call must include a concise user-facing 'title' and an explicit 'task_level' in addition to 'schedule' and 'instruction'. Choose L0 for conversation/reminders, L1 for simple tasks, L2 for multi-step coordinated tasks, L3 for complex engineering tasks, and L4 for frontier-scale or must-not-fail critical tasks. Never omit or guess an empty title or level.
