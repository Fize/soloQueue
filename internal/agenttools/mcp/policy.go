...
// definition. Approval is bound to the exact definition digest.
type Policy struct {
...
// needs_review without mutating policy history.
func (s *PolicyStore) Effective(ctx context.Context, scope string, cfg ServerConfig) (Policy, error) {
	...
	fallback := Policy{
		Scope: scope, ServerName: cfg.Name,
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT scope, server_name, state, revision, definition_digest,
		       approved_at, updated_at
		FROM mcp_policies
		WHERE scope = ? AND server_name = ?
	`, scope, cfg.Name).Scan(
		&policy.Scope, &policy.ServerName, &policy.State,
...
func (s *PolicyStore) Approve(ctx context.Context, scope string, cfg ServerConfig) (Policy, error) {
	if s == nil || s.db == nil {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO mcp_policies (
			scope, server_name, state, revision, definition_digest,
			approved_at, updated_at
		) VALUES (?, ?, ?, 1, ?, ?, ?)
	...
	`, scope, cfg.Name, PolicyApproved, digest, now, now)
	if err != nil {
		return Policy{}, err
	}
