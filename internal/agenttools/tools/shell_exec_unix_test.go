...
func mkShellTool(t *testing.T, maxOut int64) *shellExecTool {
	t.Helper()
	cfg := Config{
...
func TestShell_RewriteCommand(t *testing.T) {
	...
	rewrittenUnsupported := rewriteCommand(context.Background(), hostRuntime, unsupportedCmd)
	...
	if useRTK {
		rewrittenGit := rewriteCommand(context.Background(), hostRuntime, "git status")
