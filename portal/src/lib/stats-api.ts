...
async function statsRequest<T>(path: string, signal?: AbortSignal): Promise<T> {
  ...
  const envelope = (await response.json()) as StatsEnvelope<T>;
  if (envelope.error || !envelope.data) {
    throw new Error(
      envelope.error || "stats_invalid_response: response data is missing",
    );
  }
...
export function getStatsOverview(
  ...
  return statsRequest(`/api/stats/overview?${toSearchParams(query)}`, signal);
}
...
export function getStatsBreakdown(
  ...
  return statsRequest(`/api/stats/breakdowns?${params}`, signal);
}

export function getStatsFilters(
  query: StatsQuery,
  signal?: AbortSignal,
): Promise<StatsFilters> {
  return statsRequest(`/api/stats/filters?${toSearchParams(query)}`, signal);
}
...
export function getStatsActivity(
  ...
  return statsRequest(`/api/stats/activity?${params}`, signal);
}
...
export function getStatsEvents(
  ...
  return statsRequest(`/api/stats/events?${params}`, signal);
}
